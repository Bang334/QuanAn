...
router.get('/analytics/dashboard', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    if (startDate && endDate) {
      try {
        ...
        if (diff > 0) {
          ...
          const previousPeriodRevenue = await Payment.sum('amount', { where: previousPeriodWhereClause });
          
          // Calculate growth percentage
          if (previousPeriodRevenue > 0) {
            comparedToLastPeriod = ((totalRevenue - previousPeriodRevenue) / previousPeriodRevenue) * 100;
          }
        }
      } catch (error) {
    ...
    res.status(500).json({ message: 'Lỗi khi lấy dữ liệu bảng điều khiển phân tích', error: error.message });
  }
});
