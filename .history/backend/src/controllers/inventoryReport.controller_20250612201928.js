...
exports.getInventorySummary = async (req, res) => {
  try {
    ...
    const lowStockIngredientsResult = await sequelize.query(
      'SELECT COUNT(*) as count FROM ingredients WHERE stockQuantity < minStockLevel',
      { type: sequelize.QueryTypes.SELECT }
    );
    ...
    const lowStockIngredients = await sequelize.query(
      `SELECT id, name, stockQuantity as currentQuantity, minStockLevel as alertThreshold, 
              unit, unitPrice
       FROM ingredients
    ...
    const inventoryValueResult = await sequelize.query(
      'SELECT SUM(stockQuantity * unitPrice) as value FROM ingredients',
      { type: sequelize.QueryTypes.SELECT }
    );
