...
const calculateHourlyPayForSalary = async (salary) => {
  try {
    ...
    if (attendances.length === 0) {
      ...
      // Tìm một attendance bất kỳ hoặc tạo attendance ảo
      let attendanceId = null;
      const anyAttendance = await Attendance.findOne({
        where: {
          userId: userId,
          [Op.and]: [
            Sequelize.where(Sequelize.fn('MONTH', Sequelize.col('date')), salaryMonth),
            Sequelize.where(Sequelize.fn('YEAR', Sequelize.col('date')), salaryYear)
          ]
        }
      });
      
      if (anyAttendance) {
        attendanceId = anyAttendance.id;
      } else {
        // Tạo một attendance ảo cho mục đích tham chiếu
        const dummyAttendance = await Attendance.create({
          ...
          hoursWorked: 0,
          status: 'pending',
          note: 'Attendance ảo được tạo cho mục đích tính lương'
        });
    ...
    if (salaryDetails.length === 0) {
      ...
      for (const attendance of attendances) {
        ...
        if (!existingDetail) {
          ...
          // Tìm mức lương phù hợp cho nhân viên
          const user = await User.findByPk(salary.userId);
          if (!user) {
            console.error('User not found for salary:', salary.id);
            continue;
          }
          ...
          if (!salaryRate) {
            ...
            if (!salaryRate) {
              salaryRate = await SalaryRate.create({
                ...
                shift: attendance.shift || 'morning',
                hourlyRate: 25000,
                effectiveDate: new Date(salary.year, salary.month - 1, 1),
                isActive: true,
                note: 'Mức lương mặc định được tạo tự động'
              });
            }
