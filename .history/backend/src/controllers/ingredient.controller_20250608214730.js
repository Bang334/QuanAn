const { Ingredient, InventoryTransaction, User, IngredientPriceHistory } = require('../models');
  ...
  try {
    const ingredients = await Ingredient.findAll({
      where: {
        [Op.and]: [
          { isActive: true },
          sequelize.literal('currentQuantity <= alertThreshold')
        ]
      },
...
exports.updateIngredient = async (req, res) => {
  try {
    ...
    const oldPrice = ingredient.unitPrice;
...
exports.adjustQuantity = async (req, res) => {
  ...
  try {
    ...
    await ingredient.update({ quantity: newQuantity }, { transaction });
    ...
    if (newQuantity < parseFloat(ingredient.minQuantity)) {
