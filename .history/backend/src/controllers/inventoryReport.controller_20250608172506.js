...
exports.getInventorySummary = async (req, res) => {
  try {
    ...
    const lowStockIngredientsResult = await sequelize.query(
      'SELECT COUNT(*) as count FROM ingredients WHERE quantity < minQuantity',
      { type: sequelize.QueryTypes.SELECT }
    );
    ...
    const lowStockIngredients = await sequelize.query(
      `SELECT id, name, quantity as currentQuantity, minQuantity as alertThreshold, 
    ...
    const inventoryValueResult = await sequelize.query(
      'SELECT SUM(quantity * price) as value FROM ingredients',
      { type: sequelize.QueryTypes.SELECT }
    );
  ...
  try {
    ...
    const usageData = await sequelize.query(
      `SELECT 
       ...
       GROUP BY it.ingredientId, i.id, i.name, i.quantity, i.minQuantity, i.unit, i.price`,
      { type: sequelize.QueryTypes.SELECT }
    );
