...
exports.updateIngredient = async (req, res) => {
  try {
    ...
    console.log('Old price:', oldPrice, 'New price:', costPerUnit);
    ...
    await ingredient.update({
      ...
      currentStock: currentStock !== undefined ? parseFloat(currentStock) : ingredient.currentStock,
      minStockLevel: minStockLevel !== undefined ? parseFloat(minStockLevel) : ingredient.minStockLevel,
      costPerUnit: costPerUnit !== undefined ? parseFloat(costPerUnit) : ingredient.costPerUnit,
      supplierId: supplierId || ingredient.supplierId,
      isActive: isActive !== undefined ? isActive : ingredient.isActive
    });
    
    // Nếu giá thay đổi, lưu vào lịch sử giá
    if (costPerUnit !== undefined) {
      // Đảm bảo costPerUnit không null
      const newPrice = parseFloat(costPerUnit);
      const safeOldPrice = parseFloat(oldPrice || 0);
      
      console.log('Comparing prices:', safeOldPrice, 'vs', newPrice);
      
      // Chỉ tạo bản ghi lịch sử nếu giá thực sự thay đổi
      if (Math.abs(newPrice - safeOldPrice) > 0.001) { // Sử dụng epsilon để so sánh số thực
        console.log('Price changed significantly, creating history record');
        
        // Kiểm tra xem đã có bản ghi lịch sử giá nào cho nguyên liệu này trong 5 phút gần đây không
        const fiveMinutesAgo = new Date(new Date().getTime() - 5 * 60 * 1000);
        const recentPriceHistory = await IngredientPriceHistory.findOne({
          where: {
            ingredientId: id,
            changeDate: {
        ...
        if (!recentPriceHistory) {
          // Chỉ tạo bản ghi mới nếu không có bản ghi nào trong 5 phút gần đây
          await IngredientPriceHistory.create({
            ingredientId: id,
            oldPrice: safeOldPrice,
            newPrice: newPrice,
            changeDate: new Date(),
            changeReason: priceChangeReason || 'Cập nhật thông tin nguyên liệu',
          ...
          console.log('Price history record created');
        } else {
          console.log('Skipped creating duplicate price history record (recent record exists)');
        }
      } else {
        console.log('Price did not change significantly, skipping history record');
      }
    }
