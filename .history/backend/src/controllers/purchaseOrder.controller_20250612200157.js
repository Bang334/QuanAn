...
const updatePurchaseOrderStatus = async (req, res) => {
  try {
    ...
    switch (currentStatus) {
      case 'pending':
        isValidTransition = ['approved', 'rejected', 'cancelled'].includes(status);
        break;
      case 'approved':
        isValidTransition = ['ordered', 'cancelled'].includes(status);
        break;
      case 'ordered':
        isValidTransition = ['delivered', 'cancelled'].includes(status);
        break;
      case 'delivered':
        isValidTransition = false; // Đã giao không thể thay đổi trạng thái
        break;
      case 'rejected':
        isValidTransition = false; // Đã từ chối không thể thay đổi trạng thái
        break;
      case 'cancelled':
    ...
    // Nếu từ chối đơn hàng, yêu cầu phải có lý do
    if (status === 'rejected' && !rejectReason) {
      return res.status(400).json({
        success: false,
        message: 'Vui lòng cung cấp lý do từ chối đơn hàng'
      });
    }
