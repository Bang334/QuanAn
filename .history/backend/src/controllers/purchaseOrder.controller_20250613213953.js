    ...
    try {
      ...
      if (status === 'delivered') {
        ...
        for (const item of purchaseOrderItems) {
          ...
          if (ingredient) {
            const previousQuantity = parseFloat(ingredient.currentStock) || 0;
            const newQuantity = previousQuantity + parseFloat(item.quantity);
            
            // Tạo giao dịch kho với đầy đủ thông tin
        ...
        // Trigger menu item availability check after ingredients are received
        try {
          // Run asynchronously to avoid blocking the response
          setTimeout(async () => {
            console.log('Checking menu item availability after purchase order delivery...');
            await checkAllMenuItemsAvailability();
          }, 0);
        } catch (checkError) {
          console.error('Error checking menu availability after purchase order delivery:', checkError);
