...
const clockIn = async (req, res) => {
  try {
    ...
    // Kiểm tra xem có đang check-in quá sớm không (trước 30 phút so với giờ bắt đầu ca)
    const minutesEarly = scheduledStartMoment.diff(actualCheckInMoment, 'minute');
    
    if (minutesEarly > 30) {
      return res.status(400).json({ 
        message: `Bạn đang check-in quá sớm. Chỉ được check-in trước giờ làm việc tối đa 30 phút.`,
...
const markAbsentEmployees = async () => {
  try {
    ...
    console.log(`Đã đánh dấu vắng mặt cho ${absentRecords.length} nhân viên không chấm công`);
...
const autoRejectNearbySchedules = async () => {
  try {
    ...
    console.log(`Đã tự động reject ${rejectedSchedules.length} lịch làm việc do đăng ký quá gần thời gian bắt đầu ca`);
