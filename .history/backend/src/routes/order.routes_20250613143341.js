...
router.put('/items/:itemId/status', authenticateToken, isKitchen, async (req, res) => {
  try {
    ...
    // Sử dụng raw query để tránh lỗi 'status' ambiguous
    await sequelize.query(
      'UPDATE order_items SET status = ?, updatedAt = NOW() WHERE id = ?',
      {
        replacements: [status, itemId],
