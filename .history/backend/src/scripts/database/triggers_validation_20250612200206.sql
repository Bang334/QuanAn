...
-- Trigger kiểm tra luồng trạng thái món ăn trong đơn
CREATE TRIGGER IF NOT EXISTS before_order_item_status_update
...
BEGIN
    ...
    IF OLD.status = 'pending' AND NEW.status NOT IN ('approved', 'rejected', 'cancelled') THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Đơn đặt hàng chỉ có thể chuyển từ trạng thái pending sang approved, rejected hoặc cancelled';
    ELSEIF OLD.status = 'approved' AND NEW.status NOT IN ('ordered', 'cancelled') THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Đơn đặt hàng chỉ có thể chuyển từ trạng thái approved sang ordered hoặc cancelled';
    ELSEIF OLD.status = 'ordered' AND NEW.status NOT IN ('delivered', 'cancelled') THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Đơn đặt hàng chỉ có thể chuyển từ trạng thái ordered sang delivered hoặc cancelled';
    ELSEIF OLD.status = 'delivered' AND NEW.status != 'delivered' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Đơn đặt hàng đã giao không thể thay đổi trạng thái';
    ELSEIF OLD.status = 'rejected' AND NEW.status != 'rejected' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Đơn đặt hàng đã từ chối không thể thay đổi trạng thái';
    ELSEIF OLD.status = 'cancelled' AND NEW.status != 'cancelled' THEN
        SIGNAL SQLSTATE '45000'
...
BEGIN
    ...
    -- Nếu đơn hàng chuyển sang preparing, cập nhật bàn thành occupied
    ELSEIF NEW.status = 'preparing' AND OLD.status = 'pending' AND NEW.tableId IS NOT NULL THEN
        UPDATE tables
        SET status = 'occupied'
        WHERE id = NEW.tableId;
    END IF;
...
-- Trigger kiểm tra số lượng món ăn
CREATE TRIGGER IF NOT EXISTS before_order_item_insert_update
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM menu_items WHERE id = NEW.menuItemId AND status = 'available') THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Món ăn không tồn tại hoặc không sẵn sàng để đặt';
    END IF;
END //

-- Trigger kiểm tra khi nhận đơn hàng (đơn hàng đã phải đủ món)
CREATE TRIGGER IF NOT EXISTS before_order_update_to_preparing
BEFORE UPDATE ON orders
FOR EACH ROW
BEGIN
    -- Nếu đơn hàng chuyển từ pending sang preparing, kiểm tra đã có ít nhất 1 món
    IF OLD.status = 'pending' AND NEW.status = 'preparing' THEN
        ...
        IF item_count = 0 THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Đơn hàng phải có ít nhất một món ăn trước khi chuyển sang trạng thái preparing';
        END IF;
    END IF;
END //

-- Trigger kiểm tra khi hoàn thành đơn hàng (tất cả món phải đã được phục vụ)
CREATE TRIGGER IF NOT EXISTS before_order_update_to_completed
BEFORE UPDATE ON orders
FOR EACH ROW
BEGIN
    -- Nếu đơn hàng chuyển sang completed, kiểm tra tất cả món đã được served hoặc cancelled
    IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
        DECLARE unserved_count INT;
        
        SELECT COUNT(*) INTO unserved_count
        FROM order_items
        WHERE orderId = NEW.id 
        AND status NOT IN ('served', 'cancelled');
        
        IF unserved_count > 0 THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Tất cả các món ăn phải được phục vụ hoặc hủy trước khi hoàn thành đơn hàng';
        END IF;
    END IF;
END //
