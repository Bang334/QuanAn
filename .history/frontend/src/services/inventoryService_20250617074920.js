...
export const updateIngredient = async (id, ingredientData) => {
  try {
    ...
    if (ingredientData.costPerUnit === null || ingredientData.costPerUnit === undefined) {
      ingredientData.costPerUnit = 0;
    }
    
    console.log('Updating ingredient with ID:', id);
    console.log('Ingredient data being sent:', ingredientData);
...
// Check availability for a specific menu item
export const checkMenuItemAvailability = async (menuItemId) => {
  try {
    const response = await API.get(`/recipes/check-availability/${menuItemId}`);
    return response.data;
  } catch (error) {
    throw error.response?.data || error.message;
  }
...
export const checkAllMenuItemsAvailability = async () => {
  try {
    const response = await API.get('/recipes/check-all-availability');
    return response.data;
  } catch (error) {
    throw error.response?.data || error.message;
  }
