...
const promotionService = {
  ...
  getAllPromotions: async (filters = {}) => {
    ...
    try {
      ...
      let url = `${API_URL}/promotions/admin`;
  ...
  getPromotionsByOrder: async (orderId) => {
    const token = localStorage.getItem('token');
    try {
      const response = await axios.get(`${API_URL}/promotions/order/${orderId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  removePromotionFromOrder: async (orderId, promotionId) => {
    const token = localStorage.getItem('token');
    try {
      const response = await axios.delete(`${API_URL}/promotions/order/${orderId}/promotion/${promotionId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
