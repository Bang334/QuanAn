...
const OrderStatusPage = () => {
  ...
  const fetchOrder = async () => {
    try {
      // Chỉ hiển thị loading khi lần đầu tải
      if (!order) {
        setLoading(true);
      }
  ...
  // Tải đơn hàng khi component được mount
  useEffect(() => {
    if (!tableId) {
      navigate('/');
      return;
    ...
    // Thiết lập interval để tự động làm mới sau mỗi 30 giây
    const refreshInterval = setInterval(() => {
      // Chỉ tự động làm mới nếu đơn hàng chưa hoàn thành hoặc hủy
      if (order && order.status !== 'completed' && order.status !== 'cancelled') {
        console.log('Tự động làm mới trạng thái đơn hàng...');
        fetchOrder();
      }
    }, 30000); // 30 giây
  ...
  }, [orderId, tableId, navigate, order?.status]);
    ...
    >
      ...
      >
        ...
        {order.status !== 'completed' && order.status !== 'cancelled' && (
          <Box sx={{ 
            ...
            <Typography variant="body2" color="info.main" align="center">
              Trang sẽ tự động cập nhật trạng thái đơn hàng sau mỗi 30 giây
            </Typography>
          </Box>
        )}
        ...
        >
          ...
          {/* Nút xem chi tiết đơn hàng - có thể là để đánh giá hoặc chỉ xem chi tiết */}
          <Button 
            variant={order.status === 'completed' ? "contained" : "outlined"}
            color={order.status === 'completed' ? "primary" : "info"}
            onClick={() => navigate(`/order-detail/${order.id}`)}
            sx={{ flex: 1, mr: { sm: 2 } }}
          >
            {order.status === 'completed' ? "Xem chi tiết và đánh giá" : "Xem chi tiết đơn hàng"}
          </Button>
