  ...
  return (
    <Box sx={{ width: '100%', maxWidth: '100%', m: 0, p: 0, overflow: 'hidden' }}>
      ...
      <Drawer anchor="right" open={filterDrawerOpen} onClose={() => setFilterDrawerOpen(false)} PaperProps={{ sx: { width: '80%', maxWidth: 300, borderTopLeftRadius: 16, borderBottomLeftRadius: 16, p: 2 } }}>
        ...
        <List sx={{ pt: 1 }}>
          <ListItem button selected={category === 'all'} onClick={(e) => handleCategoryChange(e, 'all')} sx={{ borderRadius: 2, mb: 1, bgcolor: category === 'all' ? alpha(theme.palette.primary.main, 0.1) : 'transparent' }}>
          ...
          {uniqueCategories.filter(([key]) => key !== 'all').map(([key, displayName]) => (
            <ListItem button key={key} selected={category === key} onClick={(e) => handleCategoryChange(e, key)} sx={{ borderRadius: 2, mb: 1, bgcolor: category === key ? alpha(theme.palette.primary.main, 0.1) : 'transparent' }}>
      ...
      {filteredItems.length === 0 ? (
        ...
        <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
          <Grid container spacing={1} sx={{ width: '100%', m: 0, p: 0, justifyContent: 'space-between', '& > .MuiGrid-item': { height: 'auto', maxWidth: { xs: '50%', sm: '33.333%', md: '25%' }, width: { xs: '50%', sm: '33.333%', md: '25%' }, flex: { xs: '0 0 50%', sm: '0 0 33.333%', md: '0 0 25%' }, p: 1 } }}>
            {filteredItems.map((item, index) => (
              <Grid item xs={6} sm={4} md={3} key={item.id}>
                <Zoom in={true} style={{ transitionDelay: `${Math.min(index * 50, 300)}ms` }}>
                  <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 2, overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', transition: 'transform 0.3s', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 8px 24px rgba(0,0,0,0.15)' } }}>
                    ...
                    <CardContent sx={{ flexGrow: 1, p: 2, '&:last-child': { pb: 2 }, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '160px' }}>
                      <Box>
                        <Typography variant="h6" component="div" gutterBottom sx={{ fontWeight: 'bold', fontSize: '1rem', mb: 1, height: '2.4em', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                          {item.name}
                        </Typography>
                        {!isMobile && (
                          <Typography variant="body2" color="text.secondary" sx={{ mb: 1, height: '3em', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                    ...
                    <CardActions sx={{ p: 2, justifyContent: 'center', height: '50px' }}>
                      <Button startIcon={!isMobile && <AddIcon />} variant="contained" fullWidth onClick={() => handleAddToCart(item)} sx={{ borderRadius: 2, py: 0.75, minHeight: 36, fontSize: '0.9rem', fontWeight: 'bold', textTransform: 'none', boxShadow: '0 4px 8px rgba(0,0,0,0.15)' }}>
      ...
      <Snackbar open={snackbarOpen} autoHideDuration={2000} onClose={handleSnackbarClose} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }} sx={{ mb: { xs: 7, sm: 2 } }} TransitionComponent={Fade}>
        <Alert onClose={handleSnackbarClose} severity="success" variant="filled" sx={{ width: '100%', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
          {snackbarMessage}
        </Alert>
      </Snackbar>
    </Box>
