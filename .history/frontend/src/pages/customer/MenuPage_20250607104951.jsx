...
const MenuPage = () => {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: '100%', m: 0, p: 0, overflow: 'hidden' }}>
      ...
      {/* Mobile category chips for easy access */}
      ...
      {filteredItems.length === 0 ? (
        ...
        <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center', p: { xs: 0.5, sm: 1 } }}>
          <Grid container spacing={1} sx={{ width: '100%', m: 0, p: 0, justifyContent: 'center' }}>
            {filteredItems.map((item, index) => (
              <Grid key={item.id} sx={{ width: { xs: '50%', sm: '33.333%', md: '25%' }, padding: { xs: 0.5, sm: 1 } }}>
                <Zoom in={true} style={{ transitionDelay: `${Math.min(index * 30, 200)}ms` }}>
                  <Card sx={{ 
                    height: '100%', 
                    display: 'flex',
                    maxWidth: '100%',
                    ...
                    overflow: 'hidden', 
                    boxShadow: { xs: '0 2px 6px rgba(0,0,0,0.08)', sm: '0 4px 12px rgba(0,0,0,0.1)' }, 
                    transition: 'transform 0.3s', 
                    '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 8px 24px rgba(0,0,0,0.15)' }
                  }}>
                    <Box sx={{ position: 'relative', width: '100%', height: { xs: '130px', sm: '180px', md: '200px' }, overflow: 'hidden' }}>
                      <CardMedia
                        component="img"
                    ...
                    <CardContent sx={{ 
                      ...
                      <Box>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 0.5 }}>
                          ...
                          />
                          <Typography variant="caption" sx={{ ml: 0.5, color: 'text.secondary', fontSize: { xs: '0.65rem', sm: '0.75rem' } }}>
                            {item.avgRating ? `(${item.avgRating.toFixed(1)})` : '(Chưa có đánh giá)'}
                          </Typography>
                        </Box>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 0.5 }}>
                        <Typography variant="h6" color="primary" sx={{ fontWeight: 'bold', fontSize: { xs: '0.875rem', sm: '1rem' } }}>
                          {formatPrice(item.price)}
                        </Typography>
                        {item.isPopular && !isMobile && (
                          <Chip label="Phổ biến" color="error" size="small" sx={{ fontWeight: 'bold', fontSize: '0.7rem', height: 20 }} />
                        )}
                      </Box>
                    </CardContent>
                    <CardActions sx={{ p: { xs: 1, sm: 1.5 }, pt: 0, justifyContent: 'center', height: { xs: '46px', sm: '60px' } }}>
                      <Stack direction="row" spacing={1} width="100%">
                        <Button 
                          ...
                          startIcon={<InfoIcon sx={{ fontSize: { xs: '0.9rem', sm: '1rem' } }} />}
                        >
                          {isMobile ? "Chi tiết" : "Xem chi tiết"}
                        </Button>
                        <Button 
                          startIcon={!isMobile && <AddIcon sx={{ fontSize: { xs: '0.9rem', sm: '1rem' } }} />} 
                          variant="contained" 
                          size="small"
                        ...
                        >
                          {isMobile ? <AddIcon sx={{ fontSize: '0.9rem' }} /> : "Thêm vào giỏ"}
                        </Button>
                      </Stack>
                    </CardActions>
                  </Card>
