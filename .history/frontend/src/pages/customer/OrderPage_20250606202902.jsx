...
const OrderPage = () => {
  ...
  const handleRequestPayment = async (orderId) => {
    try {
      setLoading(true);
      await axios.put(`${API_URL}/api/orders/${orderId}/payment-request`, { tableId });
      const response = await axios.get(`${API_URL}/api/orders/table/${tableId}`);
      setOrders(response.data);
      setError(null);
    } catch (err) {
  ...
  if (!tableId) {
    return (
      <Box textAlign="center" py={4} sx={{ width: '100%', maxWidth: '500px', mx: 'auto' }}>
  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh', width: '100%', maxWidth: '500px', mx: 'auto' }}>
  ...
  if (error) {
    return (
      <Box sx={{ p: 3, textAlign: 'center', width: '100%', maxWidth: '500px', mx: 'auto' }}>
