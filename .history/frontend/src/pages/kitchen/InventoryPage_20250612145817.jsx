...
import { 
  ...
  History, TrendingUp, Inventory, Receipt, AddCircleOutline, RemoveCircleOutline
...
const InventoryPage = () => {
  ...
  const filteredIngredients = useMemo(() => {
    ...
    return ingredients.filter(ingredient => {
      ...
      const matchesStatus = statusFilter === 'all' || 
        (statusFilter === 'low' && parseFloat(ingredient.quantity) < parseFloat(ingredient.minQuantity)) ||
        (statusFilter === 'normal' && parseFloat(ingredient.quantity) >= parseFloat(ingredient.minQuantity));
              ...
              return (
                <TableRow key={ingredient.id}>
                  ...
                  <TableCell align="right">
                    ...
                    {parseFloat(ingredient.quantity) < parseFloat(ingredient.minQuantity) && (
                      <Tooltip title="Số lượng thấp hơn ngưỡng cảnh báo">
                        <Warning color="error" fontSize="small" sx={{ ml: 1, verticalAlign: 'middle' }} />
                      </Tooltip>
                    )}
                  </TableCell>
                  <TableCell align="right">{ingredient.minQuantity} {ingredient.unit}</TableCell>
                  <TableCell align="right">
                    {new Intl.NumberFormat('vi-VN').format(ingredient.price || ingredient.cost || 0)}
                  </TableCell>
        ...
        <Grid container spacing={2} sx={{ mt: 1 }}>
          <Grid item xs={12}>
            <TextField
              ...
              required
            >
              {/* Suppliers would be fetched from API */}
              <MenuItem value="1">Nhà cung cấp A</MenuItem>
              <MenuItem value="2">Nhà cung cấp B</MenuItem>
              <MenuItem value="3">Nhà cung cấp C</MenuItem>
            </TextField>
          </Grid>
          <Grid item xs={12}>
            <TextField
            ...
            <Grid item xs={12}>
              <Box sx={{ bgcolor: 'background.default', p: 2, borderRadius: 1 }}>
                <Typography variant="body2" gutterBottom>
                  Số lượng hiện tại: {currentItem.quantity} {currentItem.unit}
                </Typography>
                <Typography variant="body2" gutterBottom>
                  Số lượng sau điều chỉnh: {' '}
                  <strong>
                    {adjustmentForm.type === 'increase' 
                      ? (parseFloat(currentItem.quantity) + parseFloat(adjustmentForm.quantity || 0)).toFixed(2)
                      : (parseFloat(currentItem.quantity) - parseFloat(adjustmentForm.quantity || 0)).toFixed(2)
                    } {currentItem.unit}
                  </strong>
                </Typography>
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      {/* Low stock warning */}
      {tabValue === 0 && filteredIngredients.some(ing => parseFloat(ing.quantity) < parseFloat(ing.minQuantity)) && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          Có {filteredIngredients.filter(ing => parseFloat(ing.quantity) < parseFloat(ing.minQuantity)).length} nguyên liệu dưới ngưỡng cảnh báo cần được đặt thêm.
        </Alert>
      )}
