...
const KitchenAttendancePage = () => {
  ...
  const handleConfirmSchedule = async (scheduleId) => {
    ...
    try {
      // Tìm thông tin lịch làm việc để hiển thị thông báo chi tiết
      const scheduleToConfirm = schedules.find(s => s.id === scheduleId);
      ...
      // Hiển thị notification chi tiết
      if (scheduleToConfirm) {
        const scheduleDate = dayjs(scheduleToConfirm.date).format('DD/MM/YYYY');
        const scheduleShift = getShiftName(scheduleToConfirm.shift);
        
        notification.success({
          message: 'Xác nhận thành công',
          description: `Bạn đã xác nhận lịch làm việc ca ${scheduleShift} ngày ${scheduleDate} thành công`,
          placement: 'topRight',
          duration: 5,
        });
      }
      ...
      const errorMessage = error.response?.data?.message || 'Không thể xác nhận lịch làm việc';
      
      // Hiển thị thông báo lỗi
      message.error(errorMessage);
      
      // Hiển thị chi tiết lỗi trong modal
      setErrorModalTitle('Lỗi xác nhận lịch làm việc');
  ...
  const handleRejectSchedule = async () => {
    ...
    try {
      // Tìm thông tin lịch làm việc để hiển thị thông báo chi tiết
      const scheduleToReject = schedules.find(s => s.id === selectedScheduleId);
      ...
      // Hiển thị thông báo thành công
      message.success('Đã từ chối lịch làm việc thành công');
      
      // Hiển thị notification chi tiết
      if (scheduleToReject) {
        const scheduleDate = dayjs(scheduleToReject.date).format('DD/MM/YYYY');
        const scheduleShift = getShiftName(scheduleToReject.shift);
        
        notification.success({
          message: 'Từ chối thành công',
          description: `Bạn đã từ chối lịch làm việc ca ${scheduleShift} ngày ${scheduleDate} với lý do: ${rejectReason || 'Không có lý do'}`,
          placement: 'topRight',
          duration: 5,
        });
      } else {
      ...
      // Hiển thị chi tiết lỗi trong notification
      notification.error({
      ...
      // Hiển thị chi tiết lỗi trong modal
      setErrorModalTitle('Lỗi từ chối lịch làm việc');
      setErrorModalMessage(errorMessage);
      setErrorModalVisible(true);
    }
  };
  ...
  const handleCheckIn = async () => {
    // Kiểm tra xem có lịch làm việc đã xác nhận cho hôm nay không
    if (!todaySchedule || todaySchedule.status !== 'confirmed') {
      const errorMessage = 'Bạn cần có lịch làm việc đã được xác nhận cho hôm nay để chấm công';
      
      // Hiển thị thông báo lỗi
      message.error(errorMessage);
      
      // Hiển thị modal lỗi
      setErrorModalTitle('Không thể chấm công');
      setErrorModalMessage(errorMessage);
      setErrorModalVisible(true);
    ...
    try {
      await axios.post(`${API_URL}/api/attendance/check-in`, {
        scheduleId: todaySchedule.id  // Gửi ID của lịch làm việc để backend kiểm tra
      }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      ...
      // Hiển thị notification chi tiết
      const scheduleDate = dayjs(todaySchedule.date).format('DD/MM/YYYY');
      const scheduleShift = getShiftName(todaySchedule.shift);
      
      notification.success({
        message: 'Chấm công thành công',
        description: `Bạn đã chấm công vào ca ${scheduleShift} ngày ${scheduleDate} thành công`,
        placement: 'topRight',
        duration: 5,
      });
  ...
  const handleRegisterSchedule = async () => {
    ...
    try {
      ...
      // Hiển thị thông báo thành công với nhiều cách
      const successMessage = res.data?.message || 'Đăng ký lịch làm việc thành công!';
      ...
      notification.success({
        ...
        description: `Bạn đã đăng ký lịch làm việc ca ${getShiftName(registerShift)} ngày ${registerDate.format('DD/MM/YYYY')} thành công.`,
        placement: 'topRight',
        duration: 5,
      });
      ...
      const errorMessage = error.response?.data?.message || 'Đăng ký lịch làm việc thất bại!';
      
      // Hiển thị thông báo lỗi
      message.error(errorMessage);
      
      // Hiển thị chi tiết lỗi trong modal
      setErrorModalTitle('Lỗi đăng ký lịch làm việc');
      setErrorModalMessage(errorMessage);
      setErrorModalVisible(true);
    } finally {
  ...
  // Hàm chuyển đổi mã ca làm việc sang tên hiển thị
  ...
  const handleCancelSchedule = async (scheduleId) => {
    ...
    try {
      ...
      notification.success({
        ...
        description: 'Bạn đã hủy đăng ký ca làm việc thành công',
        placement: 'topRight',
        duration: 5,
      });
      ...
      const errorMessage = error.response?.data?.message || 'Hủy ca làm việc thất bại!';
      
      // Hiển thị thông báo lỗi
      message.error(errorMessage);
      
      // Hiển thị chi tiết lỗi trong modal
      setErrorModalTitle('Lỗi hủy lịch làm việc');
      setErrorModalMessage(errorMessage);
      setErrorModalVisible(true);
    }
  };
              ...
              items={[
                ...
                },
                {
                  ...
                  children: (
                    <div>
                      <Space style={{ marginBottom: 16 }}>
                        {todaySchedule && todaySchedule.status === 'confirmed' && !todayAttendance && (
                          <Button 
                            type="primary" 
                        ...
                        {todaySchedule && todaySchedule.status === 'scheduled' && !todayAttendance && (
                          <Button 
                            type="primary"
                            disabled
                            title="Bạn cần xác nhận lịch làm việc trước khi chấm công"
                          >
                            Chấm công vào ca
                          </Button>
                        )}
          ...
          <div style={{ padding: '20px 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ 
                ...
                <CloseOutlined style={{ fontSize: 20, color: '#ff4d4f' }} />
              </div>
              <Text strong style={{ fontSize: 16 }}>{errorModalTitle}</Text>
            </div>
