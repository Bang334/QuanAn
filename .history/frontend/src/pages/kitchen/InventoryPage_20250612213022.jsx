        ...
        <TableBody>
          {filteredIngredients.length > 0 ? (
            filteredIngredients.map((ingredient) => {
              ...
              // Tìm nhà cung cấp từ danh sách suppliers nếu có supplier hoặc supplierId
              let supplierName = ingredient.supplier;
              if (!supplierName && ingredient.supplierId) {
        ...
        <TableBody>
          {filteredOrders.length > 0 ? (
            filteredOrders.map((order) => {
              // Xác định tên nhà cung cấp, bảo đảm hiển thị đúng
              const supplierName = typeof order.supplier === 'string' ? order.supplier : order.supplier?.name;
              
              return (
                <TableRow key={order.id}>
                  <TableCell>#{order.id}</TableCell>
                  <TableCell>{supplierName || 'Không xác định'}</TableCell>
                  <TableCell>
