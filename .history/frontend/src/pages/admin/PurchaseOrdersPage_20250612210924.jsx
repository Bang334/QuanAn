...
const PurchaseOrdersPage = () => {
  ...
  const handleViewOrder = async (orderId) => {
    try {
      ...
      if (!orderDetails.supplier && orderDetails.supplierId) {
        try {
      ...
      if (!orderDetails.PurchaseOrderItems || orderDetails.PurchaseOrderItems.length === 0) {
        ...
        try {
          ...
          if (items && items.length > 0) {
            // Lấy thêm thông tin chi tiết cho mỗi nguyên liệu
            for (let i = 0; i < items.length; i++) {
              if (items[i].ingredientId && (!items[i].ingredient || !items[i].ingredient.name)) {
                try {
                  const ingredientDetails = await inventoryService.getIngredientById(items[i].ingredientId);
                  if (ingredientDetails) {
                    items[i].ingredient = ingredientDetails;
                  }
                } catch (ingError) {
                  console.error(`Lỗi khi lấy thông tin nguyên liệu ID ${items[i].ingredientId}:`, ingError);
                }
              }
            }
        ...
        // Lấy thêm thông tin chi tiết cho mỗi nguyên liệu nếu cần
        for (let i = 0; i < orderDetails.items.length; i++) {
          const item = orderDetails.items[i];
          if (item.ingredientId && (!item.ingredient || !item.ingredient.name)) {
            try {
              const ingredientDetails = await inventoryService.getIngredientById(item.ingredientId);
              if (ingredientDetails) {
                orderDetails.items[i].ingredient = ingredientDetails;
              }
            } catch (ingError) {
              console.error(`Lỗi khi lấy thông tin nguyên liệu ID ${item.ingredientId}:`, ingError);
            }
          }
        }
      }
        ...
        <DialogContent dividers>
          {loading ? (
            ...
            <Grid container spacing={3}>
              ...
              {/* Ghi chú */}
              {(currentOrder.notes || currentOrder.rejectReason) && (
                <Grid item xs={12}>
                  <Card variant="outlined">
                    <CardContent>
                      <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold', color: 'primary.main' }}>
                                  ...
                                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                    {item.ingredient?.image && (
                                      <Box 
                                        ...
                                        sx={{ width: 30, height: 30, mr: 1, borderRadius: '4px' }}
                                      />
                                    )}
                                    {item.ingredient?.name || 'Không có thông tin'}
                                  </Box>
                                </TableCell>
