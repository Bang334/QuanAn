...
const MenuManagementPage = () => {
  ...
  const fetchRecipeForMenuItem = async (menuItemId) => {
    try {
      console.log('Fetching recipe for menu item:', menuItemId);
      ...
      if (recipeData && recipeData.RecipeIngredients) {
        ...
        console.log('Setting ingredients from recipe:', ingredientsFromRecipe);
        setSelectedIngredients(ingredientsFromRecipe);
        calculateSuggestedPrice(ingredientsFromRecipe);
      } else {
        console.log('No recipe ingredients found, clearing selection');
        setSelectedIngredients([]);
      }
    } catch (error) {
