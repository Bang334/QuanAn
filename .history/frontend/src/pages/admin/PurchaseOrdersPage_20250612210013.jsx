...
const PurchaseOrdersPage = () => {
  ...
  const handleViewOrder = async (orderId) => {
    try {
      ...
      // Nếu không có items trong orderDetails
      if (!orderDetails.items || orderDetails.items.length === 0) {
        console.warn('Không tìm thấy danh sách nguyên liệu trong đơn hàng', orderId);
        
        // Thử lấy danh sách nguyên liệu từ purchase_order_items trực tiếp
        try {
          // Giả sử bạn có API endpoint để lấy purchase_order_items
          const itemsResponse = await fetch(`${API_ENDPOINT}/purchase-order-items?purchaseOrderId=${orderId}`);
          const itemsData = await itemsResponse.json();
          
          if (itemsData && itemsData.length > 0) {
            // Thêm items vào orderDetails
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
              ...
              {currentOrder.supplier && (
                <>
                  <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                    Địa chỉ: {currentOrder.supplier.address || 'Không có thông tin'}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    Điện thoại: {currentOrder.supplier.phone || 'Không có thông tin'}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    Email: {currentOrder.supplier.email || 'Không có thông tin'}
                  </Typography>
                </>
              )}
            </Grid>
            ...
            <Grid item xs={6}>
              ...
              <Typography variant="body2" color="textSecondary" sx={{ mt: 2 }}>Người yêu cầu:</Typography>
              <Typography variant="body1">
              ...
              {currentOrder.status !== 'pending' && currentOrder.approver && (
                <>
                  <Typography variant="body2" color="textSecondary" sx={{ mt: 1 }}>Người phê duyệt:</Typography>
                  <Typography variant="body1">
            ...
            {currentOrder.adminNotes && (
              <Grid item xs={12}>
                <Typography variant="body2" color="textSecondary">Ghi chú của Admin:</Typography>
                <Typography variant="body1">{currentOrder.adminNotes}</Typography>
              </Grid>
            )}
                  ...
                  <TableBody>
                    {currentOrder.items?.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell align="right">{item.ingredient?.unit}</TableCell>
            ...
            {currentOrder.status === 'pending' && (
              <Grid item xs={12} sx={{ mt: 2 }}>
                <Divider sx={{ my: 2 }} />
                <Typography variant="h6" sx={{ mb: 1 }}>Duyệt đơn hàng</Typography>
            ...
            filteredOrders.map((order) => {
              ...
              return (
                <TableRow key={order.id}>
                  ...
                  <TableCell>{supplierName || 'N/A'}</TableCell>
                  <TableCell>{order.requester?.name || 'N/A'}</TableCell>
                  <TableCell>
