...
const InventoryManagementPage = () => {
  ...
  const handleIngredientFormChange = (e) => {
    ...
    // Nếu đang thay đổi giá (costPerUnit) và đã có nguyên liệu hiện tại
    if (name === 'costPerUnit' && currentItem && parseFloat(value) !== parseFloat(currentItem.costPerUnit)) {
      // Hiển thị trường nhập lý do thay đổi giá
      setIngredientForm({
        ...ingredientForm,
        [name]: name === 'currentStock' || name === 'minStockLevel' || name === 'costPerUnit' 
          ? parseFloat(value) 
          : value,
        showPriceChangeReason: true,
        priceChangeReason: 'Giá đã thay đổi'
      });
    } else {
      setIngredientForm({
        ...ingredientForm,
  ...
  const handleSaveIngredient = async () => {
    try {
      ...
      const formData = {
        ...ingredientForm,
        costPerUnit: ingredientForm.costPerUnit || 0,
        currentStock: ingredientForm.currentStock || 0,
        minStockLevel: ingredientForm.minStockLevel || 0
      };
      
      // Nếu có thay đổi giá, thêm lý do thay đổi giá vào formData
      if (ingredientForm.showPriceChangeReason && currentItem) {
        if (!ingredientForm.priceChangeReason) {
          showSnackbar('Vui lòng nhập lý do thay đổi giá', 'warning');
          return;
        }
        formData.priceChangeReason = ingredientForm.priceChangeReason;
      }
      
      if (currentItem) {
        await inventoryService.updateIngredient(currentItem.id, formData);
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1 }}>
                      <Tooltip title="Xem lịch sử giao dịch">
                        <IconButton
                          color="info"
                          size="small"
                          onClick={() => navigate(`/admin/inventory/ingredient/${ingredient.id}/history`)}
                        >
                      ...
                      <Tooltip title="Xem lịch sử giá">
                        <IconButton
                          color="primary"
                          size="small"
                          onClick={() => navigate(`/admin/inventory/ingredient/${ingredient.id}/price-history`)}
                        >
    ...
    <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth>
        ...
        <TableContainer component={Paper} sx={{ 
          ...
          <Table>
            ...
            <TableBody>
              ...
              {/* Hiển thị trường nhập lý do thay đổi giá khi giá thay đổi */}
              {ingredientForm.showPriceChangeReason && currentItem && (
                <TableRow>
                  <TableCell 
                    component="th" 
                    sx={{ 
                  ...
                  <TableCell>
                    <TextField
                      ...
                      value={ingredientForm.priceChangeReason}
                      onChange={handleIngredientFormChange}
                      placeholder="Nhập lý do thay đổi giá"
                      required
                      variant="standard"
                      helperText="Giá đã thay đổi từ trước đó, vui lòng nhập lý do"
                    />
                  </TableCell>
                </TableRow>
              )}
      ...
      <DialogActions sx={{ 
        ...
        <Button 
          ...
          disabled={
            !ingredientForm.name || 
            ingredientForm.currentStock < 0 || 
            (ingredientForm.showPriceChangeReason && !ingredientForm.priceChangeReason)
          }
          sx={{
            background: 'linear-gradient(45deg, #2196F3 30%, #21CBF3 90%)',
            borderRadius: 2,
