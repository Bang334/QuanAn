...
const RecipeManagementPage = () => {
  ...
  const handleRecipeFormChange = async (e) => {
    ...
    // Khi chọn món ăn khác, tự động load nguyên liệu
    if (name === 'menuItemId' && value) {
      try {
        // Tìm thông tin món ăn
        const menuItemId = parseInt(value);
        const recipe = recipes.find(r => r.id === menuItemId);
        ...
        if (recipe && recipe.RecipeIngredients && recipe.RecipeIngredients.length > 0) {
          ...
          // Không có công thức hoặc chưa load được, thử gọi API
          try {
            const response = await axios.get(
              `${API_URL}/api/inventory/recipes/menu-item/${menuItemId}`,
              {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
              }
            );
  ...
  const calculateRecipeCost = (recipeId) => {
    ...
    if (recipe && recipe.RecipeIngredients) {
      recipe.RecipeIngredients.forEach(recipeIngredient => {
        ...
        if (ingredient) {
          totalCost += ingredient.costPerUnit * recipeIngredient.quantity;
        }
      });
    }
