...
const MenuManagementPage = () => {
  ...
  const fetchMenuItems = async () => {
    try {
      ...
      // Tải trước công thức cho tất cả các món ăn
      const preloadRecipes = async () => {
        for (const item of response.data) {
          if (!recipeCache[item.id]) {
            try {
              const recipeData = await inventoryService.getRecipeByMenuItem(item.id);
              if (recipeData && recipeData.RecipeIngredients) {
                const ingredientsFromRecipe = recipeData.RecipeIngredients.map(ingredient => {
                  const latest = ingredients.find(ing => ing.id === ingredient.ingredientId);
                  return {
                    ingredientId: ingredient.ingredientId,
                    ingredientName: latest ? latest.name : (ingredient.Ingredient ? ingredient.Ingredient.name : ''),
                    quantity: ingredient.quantity,
                    unit: latest ? latest.unit : (ingredient.Ingredient ? ingredient.Ingredient.unit : 'g'),
                    price: latest ? latest.price : (ingredient.Ingredient ? ingredient.Ingredient.costPerUnit : 0),
                    image: latest ? latest.image : (ingredient.Ingredient ? ingredient.Ingredient.image : null)
                  };
                });
                
                setRecipeCache(prev => ({
                  ...prev,
                  [item.id]: { ingredients: ingredientsFromRecipe }
                }));
              }
            } catch (err) {
              console.error(`Error preloading recipe for item ${item.id}:`, err);
            }
          }
        }
      };
      
      // Chạy tiến trình tải trước
      preloadRecipes();
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedIngredients.length > 0 && menuItemId) {
        ...
        setRecipeCache(prev => ({
          ...prev,
          [menuItemId]: { ingredients: selectedIngredients }
        }));
      }
