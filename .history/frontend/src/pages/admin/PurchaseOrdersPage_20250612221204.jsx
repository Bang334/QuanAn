...
const PurchaseOrdersPage = () => {
  ...
  const filteredOrders = purchaseOrders.filter(order => {
    ...
    if (tabValue === 3 && order.status !== 'ordered') return false;
    if (tabValue === 4 && order.status !== 'delivered') return false;
    if (tabValue === 5 && order.status !== 'completed') return false;
    if (tabValue === 6 && order.status !== 'cancelled') return false;
        ...
        <DialogContent dividers sx={{ maxWidth: '100%', width: '100%' }}>
          {loading ? (
            ...
            <Grid container spacing={2} sx={{ width: '100%' }}>
              ...
              {(currentOrder.status === 'pending' || currentOrder.status === 'approved' || currentOrder.status === 'ordered') && (
                <Grid item xs={12}>
                  <Card variant="outlined">
                    <CardContent>
                      <Typography variant="h6" gutterBottom sx={{ fontWeight: 'bold', color: 'primary.main' }}>
                          ...
                          <FormControl fullWidth required>
                            ...
                            >
                              {currentOrder.status === 'pending' ? [
                                ...
                                <MenuItem key="ordered" value="ordered">Đã đặt hàng</MenuItem>,
