...
const TableServicePage = () => {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      // Gọi API để cập nhật trạng thái thanh toán và trạng thái đơn hàng thành completed
      await axios.put(
      ...
      await axios.put(
        `${API_URL}/api/orders/${selectedTable.order.id}/status`,
        { status: 'completed' },
        {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        }
                    ...
                    {table.order && (
                      <Box sx={{ display: 'flex', gap: 1 }}>
                        ...
                        {table.order.status === 'completed' && (
                          <Chip
                            icon={<DoneAllIcon />}
                            label={`Đã thanh toán (${table.order.paymentMethod === 'cash' ? 'Tiền mặt' : 
                              table.order.paymentMethod === 'bank' ? 'Chuyển khoản' : 'Không xác định'})`}
                            color="success"
                            size="small"
                          />
                        )}
