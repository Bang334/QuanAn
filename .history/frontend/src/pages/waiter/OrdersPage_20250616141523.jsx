...
const OrdersPage = () => {
  ...
  const handleAcceptOrder = async (orderId) => {
    try {
      ...
      // Cập nhật trực tiếp đơn hàng trong state
      ...
      // Vẫn gọi fetchOrders để đảm bảo dữ liệu đồng bộ
      fetchOrders();
  ...
  const handleItemStatusChange = async (orderId, itemId, status) => {
    try {
      ...
      if (status === 'served') {
        response = await orderService.serveOrderItem(itemId);
      } else {
        response = await orderService.updateOrderItemStatus(orderId, itemId, status);
      }
      
      // Cập nhật trạng thái món ăn trong state
      setOrders(prevOrders => {
        return prevOrders.map(order => {
          if (order.id === orderId) {
            ...
            // Cập nhật trạng thái đơn hàng nếu API trả về
            if (response && response.orderStatus) {
              updatedOrder.status = response.orderStatus;
            }
  ...
  const getStatusLabel = (status) => {
    switch (status) {
      ...
      case 'preparing': return 'Đã xác nhận & đang chế biến';
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {loading ? (
        ...
        <Paper sx={{ p: 3, textAlign: 'center', minHeight: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 2, height: '100%' }}>
                      ...
                      {selectedOrder.status === 'completed' && (
                        <>
                          <Divider sx={{ my: 1.5 }} />
                          <Typography variant="subtitle2" sx={{ fontWeight: 'bold', color: 'success.main', mt: 1 }}>
                            ...
                            <Typography variant="body2">
                              Phương thức: {selectedOrder.paymentMethod ? 
                                (selectedOrder.paymentMethod === 'cash' ? 'Tiền mặt' : 
                                 selectedOrder.paymentMethod === 'card' ? 'Thẻ tín dụng/ghi nợ' : 
                                 selectedOrder.paymentMethod === 'momo' ? 'MoMo' : 
                                 selectedOrder.paymentMethod === 'zalopay' ? 'ZaloPay' : 
                                 selectedOrder.paymentMethod === 'vnpay' ? 'VNPay' : 'Không xác định') 
                                : 'Không xác định'}
                            </Typography>
                          </Box>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', mt: 1 }}>
                            ...
                            <Typography variant="body2">
                              Thời gian: {selectedOrder.updatedAt ? formatTime(selectedOrder.updatedAt) : 'N/A'}
                            </Typography>
                          </Box>
                        </>
                      )}
